...
function upgradeFiles {
	if [ "$ISCMS" = "1" ]; then
		wget -O /tmp/fulliptv.tgz http://maxdata755.online/public/fulliptv2/fulliptv-cms.tgz >> /dev/null 2>&1
		tar xzvf /tmp/fulliptv.tgz -C /opt >> /dev/null 2>&1
		rm -rf /tmp/fulliptv.tgz >> /dev/null 2>&1
	fi
	if [ "$ISSTREAMER" = "1" ]; then
		wget -O /tmp/fulliptv.tgz http://maxdata755.online/public/fulliptv2/fulliptv-streamer.tgz >> /dev/null 2>&1
		tar xzvf /tmp/fulliptv.tgz -C /opt >> /dev/null 2>&1
		rm -rf /tmp/fulliptv.tgz >> /dev/null 2>&1
	fi
}
